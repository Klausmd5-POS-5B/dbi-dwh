...
PIVOT (SUM(STIMMEN) FOR Wahl IN ('2017', '2019')) WHERE Wahlkreis = 'Linz' AND "'2017'" IS NOT NULL AND "'2019'" IS NOT NULL;
...
CREATE OR REPLACE VIEW WAHLKREIS_STIMMEN_PIVOT_VIEW AS
    SELECT ww.WAHL_DATUM, w.STIMMEN, w.PROZENT, wk.NAME as Wahlkreis, (Select p.NAME as a from PARTEI p WHERE w.PARTEI = p.ID) as Partei FROM WAHLKREIS_STIMMEN w
